...
export default function Profile() {
  ...
  const handleFileUpload = async (image) => {
    ...
    uploadTask.on("state_changed",(snapshot)=>{
      ...
      setImagePercent(Math.round(process));
    },
    (error)=>{
      setImageError(true);
    },
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <img src={formData.profilePicture || currentUser.profilePicture} alt="PFP" className="h-24 w-24 self-center cursor-pointer rounded-full object-cover mt-2" onClick={()=> fileRef.current.click()}/>
        <p className="text-sm self-center">
          {imageError?(
            <span className="text-red-600">Error  Uploading Image!</span>): imagePercent>0 && imagePercent<100 ? (
      ...
      <p className="text-green-600 mt-5" >{updateSuccess && 'Update Successfull!'}</p>
    </div>
  )
}
